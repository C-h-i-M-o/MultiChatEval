...
def upgrade() -> None:
    ...
    if "user_comments" not in tables:
        op.create_table(
            "user_comments",
            ...
            sa.ForeignKeyConstraint(
                ["response_id"],
                ["model_responses.id"],
                name="fk_user_comments_response",
            ),
            sa.ForeignKeyConstraint(
                ["user_id"],
                ["users.id"],
                name="fk_user_comments_user",
            ),
            sa.PrimaryKeyConstraint("id"),
        )

    if "comment" in _existing_columns("user_feedback"):
        op.execute(
            """
...
def downgrade() -> None:
    tables = _existing_tables()
    if "comment" not in _existing_columns("user_feedback"):
        op.add_column("user_feedback", sa.Column("comment", sa.Text(), nullable=True))

    if "user_comments" in tables:
        op.execute(
            """
