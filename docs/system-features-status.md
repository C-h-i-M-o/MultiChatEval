...
当前版本优先保证多模型、规则评分、模型级渐进展示和历史任务查询的完整链路。系统已经可以从前端发起评测请求，并由后端并发调用真实 OpenAI-compatible 模型接口；哪个模型完整回答先完成，前端就先展示哪个模型。评测任务、回答和规则评分会写入 MySQL，并可在历史任务页分页查看。
...
- “采纳”“点赞”“详情”按钮已绑定：点赞和采纳会写入或取消 `user_feedback`，详情弹窗展示评分维度、权重、命中项和反馈状态。
...
- `feedbackType`：反馈类型，当前支持 `like` 和 `accepted`。
- `comment`：可选评论。
...
- 点赞和采纳反馈已写入 `user_feedback`，并支持重复点击取消。
- LLM Judge 评分尚未写入 `evaluation_results`。
...
- 暂无评测任务创建接口测试。
- 暂无模型适配器单元测试。
- 已有规则评分器单元测试，覆盖空回答、语义相关、关键词堆砌、代码意图、JSON 格式、表格格式、安全扣分、危险请求合理拒绝、高风险专业提醒、普通问题过度拒答和隐私/凭据泄露。
- 暂无前端组件或端到端测试。
- 暂无数据库持久化相关测试。
...
1. 实现 LLM Judge，并明确 `enableJudge` 对接口返回的影响。
2. 增加反馈统计和模型推荐页面。
3. 为评测创建、模型失败分支和反馈闭环继续补充集成测试。
4. 将 `plain:` 密钥存储升级为 `enc:v1:` 加密存储。
6. 在核心流程稳定后再考虑流式输出。
