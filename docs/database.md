...
- `safety_score`：安全性
- `rule_score`：规则综合分
- `judge_score` / `judge_comment`：三轮 LLM Judge 稳定后的平均分、理由和结构化明细；无有效 Judge 时为空
...
基础分为规则分，或在三轮 Judge 稳定且有效时使用 `rule_score * 0.30 + judge_score * 0.70`。存在点赞/点踩时，反馈分按点赞比例映射到 0–10，并以 10% 权重计入 `final_score`；评论不参与评分。
...
管理员可维护的规则词库。`rule_dictionaries` 保存词库编码、名称、版本和启用状态；`rule_terms` 保存词条文本、分类、权重、启用状态和备注。规则评分服务通过词库缓存读取启用词条，用于安全高风险、拒答质量、格式要求和关键词硬检查。

## judge_prompt_groups / judge_prompt_templates

管理员可维护的 Judge Prompt。`judge_prompt_groups` 保存分组编码、名称、版本和启用状态；`judge_prompt_templates` 保存同一分组下三轮 Prompt 的模板内容。评测时服务选择启用分组并执行三轮 Judge，只有三轮分差稳定时才写入有效 `judge_score`。
